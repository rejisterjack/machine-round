...
- Support **Google sign-in**, persisted interview history, and session replay for returning users.
...
3. **Interview session** — AI agent asks 5–7 questions (mix of behavioral + technical) via **voice-only** live interview. Candidate chooses Akshay Saini, Archy Gupta, or both as panelists. Agent generates **adaptive follow-ups** based on the specific content of each answer (not a static decision tree).
...
- Text input fallback retained at the API layer for reliability testing; the product UI is voice-only.
